...
func (a *SCIMApiService) ListScimUsers(ctx context.Context, localVarOptionals map[string]interface{}) (GetUserListResponse, *http.Response, error) {
	...
	if err := typeCheckParameter(localVarOptionals["count"], "int64", "count"); err != nil {
		return successPayload, nil, err
	}
	if err := typeCheckParameter(localVarOptionals["startIndex"], "int64", "startIndex"); err != nil {
		return successPayload, nil, err
	}

	if localVarTempParam, localVarOk := localVarOptionals["filter"].([]string); localVarOk {
		localVarQueryParams.Add("filter", parameterToString(localVarTempParam, "multi"))
	}
	if localVarTempParam, localVarOk := localVarOptionals["count"].(int64); localVarOk {
		localVarQueryParams.Add("count", parameterToString(localVarTempParam, ""))
	}
	if localVarTempParam, localVarOk := localVarOptionals["startIndex"].(int64); localVarOk {
